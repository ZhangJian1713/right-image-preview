...
- **小地图 / 嵌入式 WebView：**视口框**与轴对齐**时（常见 **0°** 旋转），压暗改为 **四条 `<rect>` 条带**，不再依赖 **`fill-rule="evenodd"`** 与下层 `<img>` 的合成（部分宿主仍会把复合 path 合成错）。**有旋转**时仍用 evenodd path。见 **`docs/minimap.zh-CN.md`**（问题三）。
